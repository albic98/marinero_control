...
class MarineroControl(Node):
    ...
    def __init__(self):
        ...
        self.waypoint_subscriber = self.create_subscription(MarkerArray, "/waypoints", self.waypoint_callback, 10)

        self.waypoint_client = ActionClient(self, FollowWaypoints, '/follow_waypoints')
        self.waypoint_client.wait_for_server()

        self.take_photo_publisher = self.create_publisher(Bool, "/take_photo", 10)
        self.photo_saved_publisher = self.create_publisher(Int32, "/photo_saved", 10)
        self.photo_cmd_vel_publisher = self.create_publisher(Twist, "/cmd_vel_photo", 10)
    ...
    def odom_callback(self, msg):
        ...
        if self.take_photo:
            ...
            if home_distance > 0.5:
                ...
                self.take_photo = False
                self.take_photo_publisher.publish(Bool(data=self.take_photo))
    ...
    def goal_callback(self, msg):
        ...
        if not self.waypoints or self.current_idx >= len(self.waypoints):
            # Only override if waypoints are empty or done
    ...
    def check_goal_reached(self):
        if self.distance_from_goal <= self.position_reached_threshold and abs(self.yaw_error) <= self.orientation_reached_threshold:
            if self.waypoints and self.current_idx < len(self.waypoints):
                self.get_logger().info(f"Succeeded waypoint {self.current_idx}")
                self.current_idx += 1
                self.goal_active = False
    ...
    def trigger_photo_waypoint(self):
        wp = PoseStamped()
        wp.header.frame_id = "map"
        wp.header.stamp = self.get_clock().now().to_msg()
        wp.pose.position = self.position
        wp.pose.orientation = self.orientation
        goal_msg = FollowWaypoints.Goal(poses=[wp])
        send_goal_future = self.waypoint_client.send_goal_async(goal_msg)
        send_goal_future.add_done_callback(self.photo_goal_response)


    def photo_goal_response(self, future):
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().warn("Photo waypoint goal rejected")
            return
        self.get_logger().info("Photo waypoint goal accepted")
        goal_handle.get_result_async().add_done_callback(self.photo_result_callback)


    def photo_result_callback(self, future):
        result = future.result().result
        self.get_logger().info("Photo waypoint action completed")
    ...
    def face_boat(self):
        ...
        if abs(error_x) > 10.0 and self.boat_box_center[0] != self.image_center[0] and abs(self.pos[4]) < math.pi:
            ...
            if error_x * self.prev_error_x < 0:
                self.integral_x = 0.0
        ...
        elif abs(error_x) <= 10.0 and self.boat_box_center[0] != self.image_center[0]:
            self.camera_base_turn = 0.0

            if abs(error_y) <= 10.0 and self.boat_box_center[1] != self.image_center[1] and 1.05 < self.pos[5] < 1.95:
                self.camera_turn = 0.0
                self.take_photo = False
                self.trigger_photo_waypoint()
                ...
                # if error direction changed, clear integral
                if error_x * self.prev_error_x < 0:
                    self.integral_x = 0.0
        ...
        # Case 3: Boat not detected
            ...
            if elapsed < 30.0:
                if error_x > 0:
                    photo_vel_msg.angular.z = 0.35 * self.scale_angular_z
                else:
                    photo_vel_msg.angular.z = - 0.35 * self.scale_angular_z
            else:
                self.camera_turn = 0.0
                ...
                self.take_photo = False
                photo_vel_msg.angular.z = 0.0
                self.get_logger().warn("Boat not found after 30s -> Aborting search.")
                self.photo_saved = 2
                del self.search_start_time

        self.take_photo_publisher.publish(Bool(data=self.take_photo))
        self.photo_saved_publisher.publish(Int32(data=self.photo_saved))
        self.photo_cmd_vel_publisher.publish(photo_vel_msg)
    ...
    def velocity_callback(self, msg):
        ...
        elif self.mode_selection == 3 or self.take_photo:
    ...
    def autonomy_control(self, vel_msg):
        ...
        if self.new_goal:
            ...
            if self.stable_frames > 20:   # e.g., 20 cycles at ~20Hz
                self.new_goal = False
        ...
        ### Debugging output
        # print(f"NAV Linear X: {nav_vel_x:.2f}, NAV Angular Z: {nav_vel_z:.2f}")
        # print(f"Distance from goal: {distance_from_goal:.2f}, Yaw error: {yaw_error:.2f}")
